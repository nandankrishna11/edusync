 ...
 * Features: Better UI, Professor suggestions, Table view, Filtering
 */
import { useState, useEffect } from 'react';
import { useAuth } from '../../auth/hooks/useAuth';
import { timetableService } from '../services/timetableService';
import DepartmentSemesterOverview from './DepartmentSemesterOverview';
...
const EnhancedTimetableManagement = () => {
  ...
  const [selectedDeptSem, setSelectedDeptSem] = useState({ department: '', semester: '', section: 'A' });
  ...
  // Fetch timetable when department/semester selection changes
  useEffect(() => {
    if (selectedDeptSem.department && selectedDeptSem.semester) {
      setShowOverview(false);
      fetchTimetableForDeptSem();
    } else {
  ...
  const fetchTimetableForDeptSem = async () => {
    try {
      setLoading(true);
      setError('');
      const data = await timetableService.getSemesterTimetable(
        selectedDeptSem.department,
        selectedDeptSem.semester,
        selectedDeptSem.section
      );
      setTimetable(data);
    } catch (error) {
      setError('Failed to fetch timetable for selected department/semester');
      console.error('Error fetching department/semester timetable:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchProfessors = async () => {
    try {
      const data = await timetableService.getProfessors();
      setProfessors(data);
    } catch (error) {
  ...
  const getFilteredTimetable = () => {
    return timetable.filter(entry => {
      ...
      const matchesSemester = !filterSemester || entry.semester === parseInt(filterSemester);
      const matchesSection = !filterSection || entry.section === filterSection;
      ...
      return matchesDay && matchesDepartment && matchesSemester && matchesSection && matchesSearch;
    });
  };

  // Get unique department-semester combinations
  const getDepartmentSemesterCombinations = () => {
    const combinations = new Set();
    timetable.forEach(entry => {
      if (entry.department_code && entry.semester) {
        combinations.add(`${entry.department_code}-${entry.semester}-${entry.section || 'A'}`);
      }
    });
    return Array.from(combinations).map(combo => {
      const [dept, sem, sec] = combo.split('-');
      return { department: dept, semester: parseInt(sem), section: sec };
    }).sort((a, b) => {
    ...
    <div className="max-w-7xl mx-auto p-6 space-y-6">
            ...
            <button
              onClick={() => {
                if (selectedDeptSem.department && selectedDeptSem.semester) {
                  setFormData(prev => ({
                    ...prev,
                    department_code: selectedDeptSem.department,
                    semester: selectedDeptSem.semester,
                    section: selectedDeptSem.section
                  }));
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <select
              value={selectedDeptSem.department}
              onChange={(e) => setSelectedDeptSem(prev => ({ ...prev, department: e.target.value, semester: '' }))}
            ...
            <select
              value={selectedDeptSem.semester}
              onChange={(e) => setSelectedDeptSem(prev => ({ ...prev, semester: e.target.value }))}
              className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
              disabled={!selectedDeptSem.department}
            >
            ...
            <select
              value={selectedDeptSem.section}
              onChange={(e) => setSelectedDeptSem(prev => ({ ...prev, section: e.target.value }))}
              className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
              disabled={!selectedDeptSem.department || !selectedDeptSem.semester}
            >
            ...
            {selectedDeptSem.department && selectedDeptSem.semester && (
              <button
                onClick={() => {
                  setSelectedDeptSem({ department: '', semester: '', section: 'A' });
                  setShowOverview(true);
                }}
        ...
        {selectedDeptSem.department && selectedDeptSem.semester && (
          <div className="mt-4 p-3 bg-indigo-50 rounded-lg border border-indigo-200">
            <p className="text-sm text-indigo-800">
              <span className="font-medium">Currently viewing:</span> {departments.find(d => d.code === selectedDeptSem.department)?.name} - Semester {selectedDeptSem.semester} (Section {selectedDeptSem.section})
            </p>
          </div>
        )}
      </div>
      ...
      {/* Main Content */}
      {showOverview ? (
        <DepartmentSemesterOverview onSelectDeptSem={setSelectedDeptSem} />
      ) : (
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    ...
                    {formData.department_code && (
                      <span className="text-xs text-gray-500 ml-2">
                        (Showing {formData.department_code} department professors)
                      </span>
                    )}
                  </label>
                  <div className="space-y-2">
                    ...
                    >
                      ...
                      {getProfessorSuggestions(formData.department_code).map(prof => (
                        <option key={prof.usn} value={prof.usn}>
                          {prof.name} ({prof.usn}) - {prof.specialization}
                        </option>
                      ))}
                    </select>
                    
                    {/* Professor suggestions based on subject */}
                    {(formData.subject || formData.subject_code) && formData.department_code && (
                      <div className="bg-blue-50 p-3 rounded-lg">
                        <p className="text-xs font-medium text-blue-800 mb-2">💡 Recommended Professors:</p>
                        <div className="space-y-1">
                          {getProfessorSuggestions(formData.department_code)
                            .filter(prof => {
                              const subject = (formData.subject || formData.subject_code || '').toLowerCase();
                              return prof.specialization.toLowerCase().includes(subject.split(' ')[0]) ||
                                     commonSubjects[formData.department_code]?.some(s => 
                                       s.toLowerCase().includes(subject) && 
                                       prof.specialization.toLowerCase().includes(s.toLowerCase())
                                     );
                            })
                            .map(prof => (
                              <button
                                key={prof.usn}
                                type="button"
                                onClick={() => setFormData({ ...formData, professor_usn: prof.usn })}
                                className="block w-full text-left px-3 py-2 text-xs bg-white rounded border hover:bg-blue-50 transition-colors"
                              >
                                <span className="font-medium">{prof.name}</span>
                                <span className="text-gray-500 ml-2">({prof.specialization})</span>
                              </button>
                            ))
                          }
                        </div>
